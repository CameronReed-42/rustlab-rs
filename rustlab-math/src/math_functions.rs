...
impl VectorF64 {
    ...
    pub fn sin(&self) -> Self {
        ...
        if len >= MATH_SIMD_THRESHOLD_F64 && crate::simd::is_simd_available() {
            unsafe { simd_sin_vec_f64(self) }
        } else {
...
impl VectorF32 {
    ...
    pub fn sin(&self) -> Self {
        ...
        if len >= MATH_SIMD_THRESHOLD_F32 && crate::simd::is_simd_available() {
            unsafe { simd_sin_vec_f32(self) }
        } else {
            let result = Col::from_fn(len, |i| {
                self.inner[i].sin()
            });
...
unsafe fn simd_sin_f64(a: &ArrayF64) -> ArrayF64 {
    ...
    for chunk in 0..chunks {
        ...
        // Simple Taylor series approximation for demonstration
        // sin(x) ≈ x - x³/6 + x⁵/120
        ...
        let term1 = _mm256_mul_pd(c3, x3);
        let term2 = _mm256_mul_pd(c5, x5);
        
        let result_vec = _mm256_add_pd(x, _mm256_add_pd(term1, term2));
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn simd_sin_vec_f64(v: &VectorF64) -> VectorF64 {
    let len = v.len();
    let mut result = Col::<f64>::zeros(len);
    ...
    for chunk in 0..chunks {
        ...
        // Load from vector's contiguous memory
        let ptr = v.as_slice().unwrap().as_ptr().add(base_idx);
        ...
        // Store directly to result
        let mut results = [0.0; 4];
        _mm256_storeu_pd(results.as_mut_ptr(), result_vec);
        
        for i in 0..4 {
            result[base_idx + i] = results[i];
        }
    }
    
    // Handle remainder
    for i in 0..remainder {
        result[chunks * 4 + i] = v.inner[chunks * 4 + i].sin();
    }
...
// Continue with other vector functions...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn simd_cos_vec_f64(v: &VectorF64) -> VectorF64 {
    let len = v.len();
    let result = Col::from_fn(len, |i| {
        v.inner[i].cos()
    });
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn simd_exp_vec_f64(v: &VectorF64) -> VectorF64 {
    let len = v.len();
    let result = Col::from_fn(len, |i| {
        v.inner[i].exp()
    });
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn simd_ln_vec_f64(v: &VectorF64) -> VectorF64 {
    let len = v.len();
    let result = Col::from_fn(len, |i| {
        v.inner[i].ln()
    });
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn simd_sqrt_vec_f64(v: &VectorF64) -> VectorF64 {
    let len = v.len();
    let mut result = Col::<f64>::zeros(len);
    ...
    for chunk in 0..chunks {
        ...
        let ptr = v.as_slice().unwrap().as_ptr().add(base_idx);
        let x = _mm256_loadu_pd(ptr);
        let result_vec = _mm256_sqrt_pd(x);
    ...
    // Handle remainder
    for i in 0..remainder {
        result[chunks * 4 + i] = v.inner[chunks * 4 + i].sqrt();
    }
...
// F32 vector implementations
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn simd_sin_vec_f32(v: &VectorF32) -> VectorF32 {
    let len = v.len();
    let result = Col::from_fn(len, |i| {
        v.inner[i].sin()
    });
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn simd_cos_vec_f32(v: &VectorF32) -> VectorF32 {
    let len = v.len();
    let result = Col::from_fn(len, |i| {
        v.inner[i].cos()
    });
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn simd_exp_vec_f32(v: &VectorF32) -> VectorF32 {
    let len = v.len();
    let result = Col::from_fn(len, |i| {
        v.inner[i].exp()
    });
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn simd_ln_vec_f32(v: &VectorF32) -> VectorF32 {
    let len = v.len();
    let result = Col::from_fn(len, |i| {
        v.inner[i].ln()
    });
...
#[cfg(target_arch = "x86_64")]
#[target_feature(enable = "avx2")]
unsafe fn simd_sqrt_vec_f32(v: &VectorF32) -> VectorF32 {
    let len = v.len();
    let mut result = Col::<f32>::zeros(len);
    ...
    for chunk in 0..chunks {
        ...
        let result_vec = _mm256_sqrt_ps(x);
    ...
    // Handle remainder
    for i in 0..remainder {
        result[chunks * 8 + i] = v.inner[chunks * 8 + i].sqrt();
    }
