    ...
    ($chart:expr, $plot:expr, $text_color:expr, $x_min:expr, $x_max:expr, $y_min:expr, $y_max:expr) => {{
        ...
        for (series_index, series) in $plot.series.iter().enumerate() {
            ...
            match series.plot_type {
                PlotType::Line => {
                    let x_slice = series.x.as_slice().expect("X vector must be contiguous for plotting");
                    let y_slice = series.y.as_slice().expect("Y vector must be contiguous for plotting");
                    let data: Vec<(f64, f64)> = x_slice.iter()
                        .zip(y_slice.iter())
                        .map(|(&x, &y)| (x, y))
                        .collect();
...
where
    DB::ErrorType: 'static,
{
    // Same implementation as render_legacy_chart but without present() call
    // [This is a duplicate for now - we should refactor to avoid code duplication]
    ...
    if let Some(title) = &plot.config.title {
        chart_builder.caption(title, ("sans-serif", 20, &text_color));
    }
    ...
    // NOTE: No present() call for subplots
    Ok(())
}

// Standalone legacy chart renderer
...
// Contour plot renderer for Native backend (shared with Jupyter)
...
// Heatmap renderer for Native backend (shared with Jupyter)
...
impl PlotBackend for NativeBackend {
    ...
    fn show(&mut self) -> Result<()> {
        ...
        {
            Err(PlottingError::FeatureNotAvailable(
                "Native display not supported on this platform. Use save() instead.".to_string()
            ))
        }
    }
    ...
    fn save(&mut self, plot: &Plot, path: &Path) -> Result<()> {
        ...
        let img = ImageBuffer::<Rgb<u8>, Vec<u8>>::from_raw(
            ...
            self.height,
            self.buffer.clone()
        ).ok_or_else(|| PlottingError::Backend("Failed to create image buffer".to_string()))?;
    ...
    fn is_available(&self) -> bool {
        // Native backend is always available, though display might not work in headless environments
        true
    }
}

// 3D Surface renderer for Native backend
...
// Standalone subplot renderer (shared between backends)
...
impl NativeBackend {
    ...
    fn show_linux(&self) -> Result<()> {
        ...
        let viewers = ["xdg-open", "eog", "feh", "display", "xv", "gwenview", "eom"];
        
        let mut last_error = None;
        for viewer in &viewers {
            match Command::new(viewer)
                .arg(&path)
                .spawn()
            {
                Ok(_) => {
                    // Keep the file alive until the viewer is likely done
        ...
        // If all viewers failed, provide helpful error message
        eprintln!("Warning: Could not open image viewer. The plot was saved to: {}", path.display());
        eprintln!("To view plots in WSL or headless environments, you need to:");
        eprintln!("  1. Install an X server (like VcXsrv or Xming on Windows)");
        eprintln!("  2. Install an image viewer: sudo apt-get install eog feh or imagemagick");
        eprintln!("  3. Set DISPLAY variable: export DISPLAY=:0");
        eprintln!("Alternatively, use .save() method to save plots as PNG files.");
        
        Err(PlottingError::Backend(format!(
            "No suitable image viewer found. Plot saved to: {}. Error: {:?}", 
            path.display(), 
            last_error
        )))
    }
    ...
    fn show_windows(&self) -> Result<()> {
        ...
        Command::new("cmd")
            ...
            .spawn()
            .map_err(|e| PlottingError::Backend(format!("Failed to open image viewer: {}", e)))?;
